  ...
  for (p in 1:2) {
    for(l in 1:L) {
      ...
      right_mat <- matrix(2*c(test_g, test_g12, test_g12, test_g22), nrow = 2, ncol = 2) /
        g(sqrt(sum(sim_vals^2)), a = a1, nu = .5)
...
png('images/simu_spatial1.png', height = 550, width = 550)
image.plot(matrix(sim_f[,1], nrow = length(x)), axes = F)
dev.off()

png('images/simu_spatial2.png', height = 550, width = 550)
image.plot(matrix(sim_f[,2], nrow = length(x)), axes = F)
dev.off()
...
png('images/simu_spatial3.png', height = 550, width = 550)
image.plot(matrix(sim_f[,1], nrow = length(x)), axes = F)
dev.off()

png('images/simu_spatial4.png', height = 550, width = 550)
image.plot(matrix(sim_f[,2], nrow = length(x)), axes = F)
dev.off()

# try with varying directional measure
h_fun3 <- function(h, a1, a2, nu1, nu2, d = 2) {
  if (h[1] > 0) {
    if (h[2] > 0) {
      complex(real = 0.97)/(1 + complex(imaginary = a1^(-1) * sqrt(sum(h^2))))^(nu1 + d/2) / 
        (1 + complex(imaginary = -a2^(-1) * sqrt(sum(h^2))))^(nu2 + d/2)
    } else {
      complex(real = -0.97)/(1 + complex(imaginary = a1^(-1) * sqrt(sum(h^2))))^(nu1 + d/2) / 
        (1 + complex(imaginary = -a2^(-1) * sqrt(sum(h^2))))^(nu2 + d/2)
    }
...
png('images/simu_spatial5.png', height = 550, width = 550)
image.plot(matrix(sim_f[,1], nrow = length(x)), axes = F)
dev.off()

png('images/simu_spatial6.png', height = 550, width = 550)
image.plot(matrix(sim_f[,2], nrow = length(x)), axes = F)
dev.off()

h_fun4 <- function(h, a1, a2, nu1, nu2, d = 2) {
  if (h[1] > 0) {
    complex(real = 0.85)/(1 + complex(imaginary = a1^(-1) * sqrt(sum(h^2))))^(nu1 + d/2) /  
      (1 + complex(imaginary = -a2^(-1) * sqrt(sum(h^2))))^(nu2 + d/2)
  } else {
    complex(real = 0.85)/(1 + complex(imaginary = -a1^(-1) * sqrt(sum(h^2))))^(nu1 + d/2) / 
      (1 + complex(imaginary = a2^(-1) * sqrt(sum(h^2))))^(nu2 + d/2)
...
set.seed(22^2)
sim_f <- simu_bivariate(grid, L = 2000, nu1 = .8, nu2 = 1.2, g_new = g_new, cc_fun = h_fun4)
image.plot(matrix(sim_f[,1], nrow = length(x)), axes = F)
image.plot(matrix(sim_f[,2], nrow = length(x)), axes = F)


png('images/simu_spatial7.png', height = 550, width = 550)
image.plot(matrix(sim_f[,1], nrow = length(x)), axes = F)
dev.off()

png('images/simu_spatial8.png', height = 550, width = 550)
image.plot(matrix(sim_f[,2], nrow = length(x)), axes = F)
dev.off()

set.seed(23^2)
sim_f <- simu_bivariate(grid, L = 1000, nu1 = 1.5, nu2 = 1.5, a1 = .2, a2 = 1.2,
                        g_new = g_new, cc_fun = h_fun4)
image.plot(matrix(sim_f[,1], nrow = length(x)), axes = F)
image.plot(matrix(sim_f[,2], nrow = length(x)), axes = F)


png('images/simu_spatial9.png', height = 550, width = 550)
image.plot(matrix(sim_f[,1], nrow = length(x)), axes = F)
dev.off()

png('images/simu_spatial10.png', height = 550, width = 550)
image.plot(matrix(sim_f[,2], nrow = length(x)), axes = F)
dev.off()

set.seed(24^2)
sim_f <- simu_bivariate(grid, L = 1000, nu1 = 0.8, nu2 = 1.2, a1 = .8, a2 = 1.2,
                        g_new = g_new, cc_fun = h_fun4)
image.plot(matrix(sim_f[,1], nrow = length(x)), axes = F)
image.plot(matrix(sim_f[,2], nrow = length(x)), axes = F)


png('images/simu_spatial11.png', height = 550, width = 550)
image.plot(matrix(sim_f[,1], nrow = length(x)), axes = F)
dev.off()

png('images/simu_spatial12.png', height = 550, width = 550)
image.plot(matrix(sim_f[,2], nrow = length(x)), axes = F)
dev.off()

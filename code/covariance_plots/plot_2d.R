...
df <- fft_2d(grid_info, nu1 = .5, nu2 = 1.5, a1 = 1, a2 = 1, Psi = Psi, d = 2, Mu = Mu)
...
df <- fft_2d(grid_info, nu1 = 1, nu2 = 1, a1 = 1.2, a2 = .8, Psi = Psi, d = 2, Mu = Mu)
ggplot(data = filter(df, abs(Var1) < 5, abs(Var2) < 5), aes(x = Var2, y = Var1, fill = val)) +
  geom_raster() + coord_equal() + 
  color_scale + labels + theme_use
ggsave('images/cc_fun_2d_9.png', height = 5.1, width = 4, dpi = 150)

df <- fft_2d(grid_info, nu1 = .5, nu2 = 1.5, a1 = 1.1, a2 = .9, Psi = Psi, d = 2, Mu = Mu)
ggplot(data = filter(df, abs(Var1) < 5, abs(Var2) < 5), aes(x = Var2, y = Var1, fill = val)) +
  geom_raster() + coord_equal() + 
  color_scale + labels + theme_use
...
df <- fft_2d(grid_info, nu1 = 1.5, nu2 = 1.5, a1 = 1, a2 = 1, Psi = Psi, d = 2, Mu = Mu)
ggplot(data = filter(df, abs(Var1) < 5, abs(Var2) < 5), aes(x = Var2, y = Var1, fill = val)) +
  geom_raster() + coord_equal() + 
...
df <- fft_2d(grid_info, nu1 = .4, nu2 = 2.5, a1 = 1, a2 = 1, Psi = Psi, d = 2, Mu = Mu)
ggplot(data = filter(df, abs(Var1) < 5, abs(Var2) < 5), aes(x = Var2, y = Var1, fill = val)) +
  geom_raster() + coord_equal() + 
  scale_fill_gradient2() + labels + theme_use
...
df1 <- fft_2d(grid_info, nu1 = 1.5, nu2 = 1.5, a1 = 1, a2 = 1, Psi = Psi, d = 2, Mu = Mu) %>%

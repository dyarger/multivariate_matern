...
full_function <- function(grid_info, nu, a, norm_type = 'A') {
  fft_1d(grid_info, nu1 = nu, nu2 = nu, a1 = a, a2 = a, Sigma_re = 0, Sigma_im = 1, norm_type = norm_type)[,2]
}
...
ggplot(data = df %>%
  ...
  geom_line(linewidth = line_cex) + 
  labs(x = 'Lags', y = 'Cross-covariance\nfunction', color = expression(a),
       linetype = expression(a))
ggsave('images/example_fun_range.png', height = 3, width = 6)

width <- 6
nu_vals <- c(.2, .5, .8, 1.1, 1.8, 2.8)
whitt_vals <- sapply(nu_vals, fft_1d, grid_info = grid_info, Sigma_re = 1, Sigma_im = 0,
...
avals <- c(.2, .5, 1, 2)
whitt_vals <- sapply(avals, fft_1d, grid_info = grid_info, Sigma_re = 1, Sigma_im = 0,
...
cov_val_lag <- fft_1d(grid_info, nu1 = .75, nu2 = .75, a1 = 1, a2 = 1, Sigma_re = 1, Sigma_im = 0, norm_type = 'A')[,2]
cov_val_lag2 <- fft_1d(grid_info, nu1 = .75, nu2 = .75, a1 = 1, a2 = 1, Sigma_re = 0, Sigma_im = 1, norm_type = 'A')[,2]
cov_val_lag3 <- fft_1d(grid_info, nu1 = .75, nu2 = .75, a1 = 1, a2 = 1, Sigma_re = sqrt(2)/sqrt(3), Sigma_im =  1/sqrt(3), norm_type = 'A')[,2]
cov_val_lag4 <- fft_1d(grid_info, nu1 = .75, nu2 = .75, a1 = 1, a2 = 1, Sigma_re = 1/sqrt(3), Sigma_im = sqrt(2)/sqrt(3), norm_type = 'A')[,2]
...
cov_val_lag <- fft_1d(grid_info, nu1 = nu_vals[1], nu2 = nu2, a1 = a1, a2 = a2, Sigma_re = re_z, Sigma_im = im_z, norm_type = 'A')[,2]
cov_val_lag2 <- fft_1d(grid_info, nu1 = nu_vals[2], nu2 = nu2, a1 = a1, a2 = a2, Sigma_re = re_z, Sigma_im = im_z, norm_type = 'A')[,2]
cov_val_lag3 <- fft_1d(grid_info, nu1 = nu_vals[3], nu2 = nu2, a1 = a1, a2 = a2, Sigma_re = re_z, Sigma_im = im_z, norm_type = 'A')[,2]
cov_val_lag4 <- fft_1d(grid_info, nu1 = nu_vals[4], nu2 = nu2, a1 = a1, a2 = a2, Sigma_re = re_z, Sigma_im = im_z, norm_type = 'A')[,2]
...
test_vals <- sapply(nu_vals, function(x) {
  fft_1d(grid_info, nu1 = x, nu2 = nu2, a1 = a1, a2 = a2, Sigma_re = 0, Sigma_im = 1, norm_type = 'A')[,2]
})
...
test_vals <- sapply(a_vals, function(x) {
  fft_1d(grid_info, nu1 = 1.25, nu2 = 1.25, a1 = x, a2 = 1, Sigma_re = 0, Sigma_im = 1, norm_type = 'A')[,2]
})
